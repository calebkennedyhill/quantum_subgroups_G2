...
% skein diagrams
\newcommand{\skein}[2]{\raisebox{-.4\height}{ \includegraphics[scale = #2]{./#1.eps}}}
...
Need:{\color{red}{Disclosures and data availabilty}}
...
\]
for some category $\DD$ in which explicit calculations are more easily performed by computers.
...
We view this new morphism as an I 
\[
    \skein{dec_tet_5}{0.15}     
\]
with an oriented red vertical strand.
In the case of $\GG_2(q)$, the properties of this new morphism were unknown beyond a few basics deriving from, e.g.,
the fact that it is a projection onto a simple object contained in the tensor square of the $\otimes$-generating object.
Once we have our hands on the image in the GPA of this projection, 
...
\begin{definition}
    ...
    For instance, 
    \[
    \Right^1\left( \skein{dec_tet_4}{0.1} \right) = \skein{dec_tet_3}{0.1} 
    \quad\text{and}\quad 
    \Right^2\left( \skein{dec_tet_4}{0.1} \right) = \skein{dec_tet_4}{0.1}.
    \] 
    ...
    For example,
    \[
        \Dec_1\left(\skein{trivalent}{0.1}\right) = \skein{triv_rightDown}{0.1},
    \quad
    \text{and} 
    \quad
        \sum_{i=1}^{3} i\Dec_i\left(\skein{trivalent}{0.1}\right) 
        = \skein{triv_rightDown}{0.1} 
        +2\skein{triv_bottomLeft}{0.1} 
        +3\skein{triv_leftUp}{0.1}
    \]
...
\begin{definition}\label{def:zn-ext}
    ...
    \begin{equation*}\tag{Schur 0}
        \skein{schur_0_1}{0.1} = 0 \quad 
        \skein{schur_0_2}{0.1} = 0 \quad \cdots \quad 
        \underbrace{ \skein{schur_0_n-1}{0.11} }_{\text{$n-1$}} = 0
    \end{equation*}

    \begin{equation*}\tag{Schur 1}
        \skein{schur_1_1}{0.1} = 0 \quad 
        \skein{schur_1_2}{0.1} = 0 \quad \cdots \quad 
        \underbrace{ \skein{schur_1_n-1}{0.11} }_{\text{$n-1$}} = 0
    \end{equation*}

    \begin{equation*}\tag{Swap}
        \skein{swap_LHS}{0.15} 
        = \omega \skein{swap_RHS}{0.15}
    \end{equation*}

    \begin{equation*}\tag{decStick}
        \skein{stick_w_Pg}{0.25} 
        = \skein{stick}{0.1}
    \end{equation*} 

    \begin{equation*}\tag{decBigon}
        \skein{g_bigon2}{0.1} = b \skein{stick}{0.15}
    \end{equation*}

    \begin{equation*}\tag{Change of Basis}
        \skein{triv_leftUp}{0.1} 
        = \sum_{i=0}^{n-1} r_i \skein{triv_rightUp_i}{0.1}
    \end{equation*}

    \begin{equation*}\tag{decTrigon}
        \skein{dec_trigon_LHS}{0.1} 
        = \sum_{i=0}^{n-1} t_i \skein{triv_rightUp_i}{0.1}
    \end{equation*}\label{eq:decTrigon}
 
    \begin{equation*}\tag{decTetragon}
         \skein{dec_tet_LHS}{0.1} 
         = \sum_{i=0}^4 \sum_{j=0}^3 u_{i,j} \Dec_i\left( \Right^j\left( \skein{dec_tet_1}{0.1} \right) \right)
         + \sum_{i=0}^4 \sum_{j=0}^3 v_{i,j} \Dec_i\left( \Right^j\left( \skein{dec_tet_4}{0.1} \right) \right)
    \end{equation*}

     \begin{equation*}\tag{decPentagon}
        \skein{dec_pent_LHS}{0.1} 
        = \sum_{i=0}^5 \sum_{j=0}^4 w_{i,j} \Dec_i\left( \Right^j\left( \skein{dec_pent_RHS1}{0.1} \right) \right) 
        + \sum_{i=0}^5 \sum_{j=0}^4 x_{i,j} \Dec_i\left( \Right^j\left( \skein{dec_pent_RHS2}{0.1} \right) \right)
    \end{equation*}
    for $\omega, b, r_i, t_i, u_{i,j}, v_{i,j}, w_{i,j}, x_{i,j} \in \C$.
    We additionally enforce the condition that the diagrams $\skein{triv_rightUp_i}{0.075}$ 
...
\begin{definition}\label{def:D4}
    ...
    \begin{equation*}
        \skein{swap_LHS}{0.1} = - \skein{swap_RHS}{0.1}, \qquad
        \skein{g_bigon2}{0.1} = (q_4+1+q_4^{-1}) \skein{stick}{0.1}
    \end{equation*}

    \begin{equation*}
        \skein{triv_leftUp}{0.1} =  
        q_4^{-4} \skein{trivalent}{0.1} 
        + q_4^{16} \skein{triv_rightUp}{0.1}
    \end{equation*}
    ...
    \begin{equation*}
        \skein{dec_trigon_LHS}{0.1} = 
        -\skein{trivalent}{0.1} 
        -\skein{triv_rightUp}{0.1}
    \end{equation*}


    \begin{align*}
         \skein{dec_tet_LHS}{0.12} & = q^2 \skein{dec_tet_1}{0.1} + q_4^2 \skein{dec_tet_2}{0.1} 
            + \frac{q_4^{17}}{q-q^{-1}} \skein{dec_tet_3}{0.1} + q_4^2 \skein{dec_tet_4}{0.1} \\
        & + \frac{1+[3]_{q_4}}{q_4^4} \skein{dec_tet_5}{0.1} + \frac{[2]_{q_4}}{q_4^{13}} \skein{dec_tet_6}{0.1}
            + q_4^{-14} \skein{dec_tet_8}{0.1} + \frac{[2]_{q_4}}{q_4^{13}} \skein{dec_tet_9}{0.1}
            + (-1) \skein{dec_tet_10}{0.1}
    \end{align*}
...
with structure constants found in the Mathematica files associated to this project.
...
this material is well known, and detailed proofs can be found in \cite{monoidalFunctorsAndAlgebras}.
...
\begin{definition}
    ...
    We say $A$ is an {\bf etale} algebra if it is commutative and separable.
    We call $A$ {\bf connected} if it is etale and $\dim\Hom_\CC(\unit \to A)=1$.
\end{definition}


For an etale algebra object $A$ of $\CC$, we denote by $\CC_A$ the category of right $A$-modules internal to $\CC$.
...
Being etale is the same thing as multiplication having a section, which means we can pop $A$-bigons.
Connected means there is only one $A$-cap and one $A$-cup.
This lets us wiggle enough to prove that the monoidal product $M\otimes_A N$ 
of right $A$-modules is well-defined.

Furthermore, when $\CC$ is semisimple, the free functor 
\[
    \FF_A: \CC \xhookrightarrow{X\mapsto X\otimes A} \CC_A
\]
is faithful monoidal but, as we will see, not always full.
...
First, we note that the target of a monoidal functor is equivalent to some category of modules.
...
Next a well-known fact about $\dagger$-functors into unitary categories.
...
One result which will help immensely in computing GPA embeddings is the following,
...
\begin{lemma}[Half-braid]\label{lem:general-half-braid}
    ...
    \begin{equation*}\tag{Half-braid}
        \skein{arb_hb_top}{0.2} = \skein{arb_hb_bottom}{0.2}
    \end{equation*}
\end{lemma}
...
We direct the reader to \cite{DMNO} for a more complete treatment.
...
\begin{remark}\label{rem:Pg-properties}
    ...
    The behavior of the $P_{g_k}$ will be captured by $\skein{dec_tet_5}{0.1}$ in $\DD_k$.
...
Recall the theory of rigid monoidal categories detailed in \cite{KW}. 
...
We also assume pivotality throughout, which gives us the freedom to isotope diagrams. 
The generators we will use for our planar algebras will be symmetrically self-dual,
thus freeing us from having to keep track of isotopy deformation of diagrams.

We call a symmetrically self-dual object $X$ a {\bf tensor generator} for the tensor category $\CC$ 
if every object of $\CC$ is isomorphic to a subobject of some tensor power $X^{\otimes n}$. 
Let $\PP_{X;\CC}$ be the full monoidal subcategory of $\CC$ whose objects are 
tensor powers $\unit=X^{\otimes0},X,X^{\otimes2},\dots$; 
...
We will be presenting the two quantum subgroups as extensions of $\GG_2(q)$ skein theories, 
in the sense of Kuperberg \cite{Kuperberg,tricats}. 
...
\begin{definition}
    ...
    unoriented trivalent vertex $\skein{trivalent}{0.05}$ satisfying the relations
    \begin{equation*}\tag{Loop}
        \skein{loop}{0.1} = q^{10} + q^{8} + q^{2} + 1 + q^{-2} + q^{-8} + q^{-10}
    \end{equation*}

    \begin{equation*}\tag{Lollipop}\label{eq:Lolli}
        \skein{lollipop}{0.1} = 0  
    \end{equation*}

    \begin{equation*}\tag{Rotate}\label{eq:Rotate}
        \Right^1\left( \skein{trivalent}{0.1} \right) = \skein{trivalent}{0.1}
    \end{equation*}

    \begin{equation*}\tag{Bigon}\label{eq:Bigon}
        \skein{bigon}{0.1} = \kappa^2 \skein{stick}{0.1}
    \end{equation*}

    \begin{equation*}\tag{Trigon}\label{eq:Trigon}
        \skein{trigon_LHS}{0.1} = -(q^4 +1+ q^{-4}) \skein{trivalent}{0.1}
    \end{equation*}

    \begin{equation*}\tag{Tetragon}\label{eq:Tetragon}
        \skein{tet_LHS}{0.1} 
        = (q^2 + q^{-2}) \left( \skein{dec_tet_1}{0.1} 
        + \skein{dec_tet_2}{0.1} \right) 
        + (q^2 +1+ q^{-2}) \left( \skein{dec_tet_3}{0.1} 
        + \skein{dec_tet_4}{0.1} \right)
    \end{equation*}

    \begin{equation*}\tag{Pentagon}\label{eq:Pentagon}
        \skein{pentagon}{0.1} = 
        - \sum_{i=0}^4 \Right^i \left( \skein{dec_pent_RHS1}{0.1} \right) 
        - \sum_{i=0}^4 \Right^i \left( \skein{dec_pent_RHS2}{0.1} \right)
    \end{equation*}
\end{definition}
...
Recall that the {\bf idempotent completion} of any category $\CC$ consists of pairs $(Z,p)$,
where $p\in\End_\CC(Z)$ is an idempotent.
We denote the idempotent completion of $\CC$ as $\Idemp(\CC)$.
Further, we define the {\bf additive envelope} of a pivotal category $\CC$
to have objects formal direct sums $\bigoplus_i Z_i$ for objects $Z_i$ of $\CC$
and morphisms $\bigoplus_i Z_i \to \bigoplus_j Z'_j$ consisting of matrices with
$(k,l)$-entry a morphism $Z_k \to Z'_l$.
...
By \cite[Corollary 2.20]{bodish_triple_clasp_g2} and \cite[Corollary 6.7]{reconstructing_g2}, 
...
\begin{itemize}
    ...
    \item Implying unitarity for semisimple quotients of the skein theories,
\end{itemize}
...
\begin{definition}\label{def:GPA}
    Let $\Gamma=(V,E)$ be a finite undirected graph. 
...
With the new strand type, we have another measure: number of directed strands. 
...
\begin{definition}
    In a trivalent category we define a {\bf box space} $B(k,l)$ to be the span of diagrams $k\to0$ with $l$ internal faces.
...
We will assume a loop parameter of $\delta$ throughout.
...
\begin{lemma}
    ...
    \begin{equation*}\tag{$\Z_n$}
        \underbrace{ \skein{n_g_strands}{0.08} }_{\text{$n$}} 
        = \skein{dec_tet_3}{0.2}
    \end{equation*}

    \begin{equation*}\tag{Split}
        \underbrace{ \skein{split_LHS}{0.125} }_{\text{$n$}} 
        = \frac{1}{\delta^2} \underbrace{ \skein{split_RHS}{0.085} }_{\text{$n$}}
    \end{equation*}
...
\begin{lemma}
    In a $\Z_n$-like extension, there exist $n$ scalars $s_i \in \C$ such that the following relation holds:
    \begin{equation*}\tag{Slide}
        \skein{slide_LHS}{0.1} 
        = \sum_{i=0}^{n-1} s_i \skein{slide_RHS2}{0.1}^i
    \end{equation*}
\end{lemma}
\begin{proof}
...
\begin{proof}
    ...
    The $m=1$ case is handled by (Change of Basis), (decStick), and (Schur 0). 
...
\[
F\left( \skein{trivalent}{0.08} \right) \in \Hom_{\GPA(\Gamma)}(2\to1).
\]
This amounts to giving a list of $M\coloneqq\tr(\Gamma^2\cdot\Gamma)$ complex numbers, $a_1,\dots,a_M$. 
Note that we freely switch between using $\Gamma$ to mean the graph itself and the graph's adjacency matrix.
...
of polynomials in the $a_i$. This assumption is useful only if it turns out to help us solve the system. 
In fact, any assumptions we make about this system, if they yield solutions, are valid.
This assumption of real coefficients has precedent \cite{Cain_Dan}.
...
\[
F\left( \skein{dec_tet_5}{0.08} \right) \in \Hom_{\GPA(\Gamma)}(2\to2)
\]
...
\begin{proof}
    ...
    \]
    be the defining basis for $\Hom_{\GPA(\Gamma_4)}(2\to1)$; at level 4 we have $M=88$. 
    See the attached Mathematica files for the specific ordering chosen.
    Then it must be that 
    \[
        F_4 \left( \skein{trivalent}{0.08} \right) = a_1(p_1,q_1)+\cdots+ a_M(p_M,q_M)
    \]
...
They, along with the linear solution, reduce many of the high-order equations to linear. 
...
\]
Up to sign, the coordinates of $F_4 \left( \skein{trivalent}{0.08} \right)$ 
...
\end{align*}
These coordinates of $F_4 \left( \skein{trivalent}{0.08} \right)$ give us a definition of $F_4$.
...
With our embedding of $\GG_2(q_4)$, i.e. the coordinates of $F_4 \left( \skein{trivalent}{0.08} \right)$, in hand, 
...
\]
which takes the simple $\otimes$-generator $X_4$ to a simple $\otimes$-generator $Y_4\coloneq \FF_{A_4}(X_4)$.
...
the role will be played by $\skein{dec_tet_5}{0.08}$.
...
\[
    F_4\left( \skein{dec_tet_5}{0.08} \right) \in \Hom_{\GPA(\Gamma_4)}(2\to2)
\]
satisfying (Recouple), (Schur 0, 1), (Swap), and (decStick).
We follow a process similar to solving for the image $F_4\left( \skein{trivalent}{0.08} \right)$.
That is, we assume that for some $b_1,\dots,b_N \in \C$ where $N \coloneq \tr(\Gamma_4^2 \cdot \Gamma_4^2) =400$ 
at level 4, we have
\[
    F_4\left( \skein{dec_tet_5}{0.08} \right) = \sum_{i=1}^N b_i (p'_i,q'_i).
\]
...
In the present context, this lemma states that our projection $\skein{dec_tet_5}{0.08}$ 
satisfies the relation
\begin{equation*}\tag{Half-braid}
    \skein{hb_top}{0.11} = \skein{hb_bottom}{0.15}
\end{equation*}
...
Solving these new equations gives us the coordinates of the projection $F_4\left( \skein{dec_tet_5}{0.08} \right)$.
...
\[
    F_4\left( \skein{swap_LHS}{0.15} \right) = \omega F_4\left( \skein{swap_RHS}{0.15} \right)
\]
...
\[
F_4\left( \skein{dec_trigon_LHS}{0.1} \right)= 
        t_1 F_4\left( \skein{trivalent}{0.1} \right) 
        + t_2 F_4\left( \skein{triv_rightUp}{0.1} \right)
\]
for $t_1$ and $t_2$.
The same principle applies when searching for the relations (decTetragon) and (decPentagon).
In practice, discovering that, e.g., the coefficient of the summand \skein{dec_pent_RHS1}{0.07}
in the (decPentagon) relation is $q^8 - q^4 + \frac{1}{q^4} + \frac{1}{q^2}$ 
required the use of some nontrivial computational algebraic number theory techniques.
For more details, see the author's PhD thesis \cite{hill_thesis}.
...
For now, we turn to a discussion of level 3.
...
so we exclusively use numerical approximations. 
The actual numbers are relegated to the Mathematica files.
...
$F_3 \left( \skein{trivalent}{0.08} \right)$.
...
\begin{proof}[Proof of Theorem~\ref{thm:level-4}]
    ...
    \[
        1_{ \ol{\GG_2(q_4)}} \mapsto V_{\Lambda_1} \mapsto V_{\Lambda_1} \otimes B_4 .
    \]
    If it was true that $B_4\cong \unit$, we would have 
    \[
        \dim \Hom_{\ol{ \Rep(U_{q_4}(\gg_2))}_{B_4}}((V_{\Lambda_1} \otimes B_4)^{\otimes 2} \to V_{\Lambda_1} \otimes B_4) = 1.
    \]
    ...
    Now we note that
    \begin{align*}
        \dim \Hom_{(\CC_4)_{B_4}}((V_{\Lambda_1} \otimes B_4)^{\otimes 2} \to V_{\Lambda_1} \otimes B_4) & = 
...
The author thanks Cain Edie-Michell for the idea to pursue this project,
and for many helpful conversations throughout its completion.
Part of this work was completed while employed as a research assistant 
by the University of New Hampshire.


\subsection*{Data Availability}
All code used to prepare this work can be accessed at the author's GitHub:  
\begin{center}
    \href{https://github.com/calebkennedyhill/quantum_subgroups_G2}
    {https://github.com/calebkennedyhill/quantum\_subgroups\_G2}.
\end{center}
All calculations can be recreated with no modification of the files contained therein.
...
The author has no relevant competing financial or non-financial 
